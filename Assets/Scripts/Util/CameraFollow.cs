...
public class CameraFollow : MonoBehaviour
{
    ...
    private void LateUpdate()
    {
        ...
        {
            desiredPosition += ViewportDeltaToWorldDelta(new Vector3(targetViewportPosition.x - horizontalDeadZone.x, 0f, targetViewportPosition.z));
        }
        else if (targetViewportPosition.x > horizontalDeadZone.y)
        {
            desiredPosition += ViewportDeltaToWorldDelta(new Vector3(targetViewportPosition.x - horizontalDeadZone.y, 0f, targetViewportPosition.z));
        }

        if (targetViewportPosition.y < verticalDeadZone.x)
        {
            desiredPosition += ViewportDeltaToWorldDelta(new Vector3(0f, targetViewportPosition.y - verticalDeadZone.x, targetViewportPosition.z));
        }
        else if (targetViewportPosition.y > verticalDeadZone.y)
        {
            desiredPosition += ViewportDeltaToWorldDelta(new Vector3(0f, targetViewportPosition.y - verticalDeadZone.y, targetViewportPosition.z));
        }
